//внедрение библиотеки в чужой процесс
#include <Windows.h>
#include <winerror.h>
...
int main(int argc, char* argv[])
{
    ...
    // 0. Open The process //открытие процесса с правами создания потока, записи в память, рабы с памятью       pid процесса
    HANDLE hProcess = OpenProcess(PROCESS_CREATE_THREAD | PROCESS_VM_WRITE | PROCESS_VM_OPERATION, FALSE, atoi(argv[1]));

    // 1. Alocate heap memory in remote process     //выделение памяти в том процессе
    ...
    CreateRemoteThread(hProcess, NULL, 0, lpLoadLibraryStartAddress, lpHeapBaseAddress1, 0, NULL);//создание удалённого потока
