...
BOOL CreateProcessWithBlockDllPolicy(IN LPSTR lpProcessPath, OUT DWORD* dwProcessId,    //путь, id
    ...
    RtlSecureZeroMemory(&Pi, sizeof(PROCESS_INFORMATION));  //очищение структур
    ...
    if (!InitializeProcThreadAttributeList(pAttrBuf, 1, NULL, &sAttrSize)) { //инициализация атрибутов процесса
        printf("[!] InitializeProcThreadAttributeList Failed With Error : %d \n",
            GetLastError());
        return FALSE;
    }
    DWORD64 dwPolicy =
        PROCESS_CREATION_MITIGATION_POLICY_BLOCK_NON_MICROSOFT_BINARIES_ALWAYS_ON;  //блокировка dll, которые не подписаны microsoft

    if (!UpdateProcThreadAttribute(pAttrBuf, NULL,      //применение политики к процессу
    ...
    if (!CreateProcessA(NULL,lpProcessPath,NULL,NULL,FALSE,EXTENDED_STARTUPINFO_PRESENT,NULL,NULL,&SiEx.StartupInfo,&Pi)) { //запуск процесса с заданными атрибутами с защитой от левых библиотек
...
DWORD GetProcessIdByName(const std::wstring& processName) { //происк процесса по имени
    DWORD pid = 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    if (strcmp(lpCmdLine, STOP_ARG) != 0) {     //если не bam
